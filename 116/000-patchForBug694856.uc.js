// ==UserScript==
// @name           000-patchForBug694856.uc.js
// @namespace      http://space.geocities.yahoo.co.jp/gl/alice0775
// @description    Workaround Bug 694856 context menus are missing for url with /
// @author         Alice0775
// @include        main
// @compatibility  116
// @version        2023/07/06 fix popupshowing event
...
var bug694856 = {
  ...
  popupshowing: function(event) {
    let linkText = gContextMenu.selectionInfo.text;
    if (!/@/.test(linkText) && /[\w\/:%#\$&\?\(\)~\.=\+\-]+\.[\w\/:%#\$&\?\(\)~\.=\+\-]+/i.test(linkText)) {
      let url = null;
      try {
        let URIFixup = Components.classes['@mozilla.org/docshell/uri-fixup;1']
                       .getService(Components.interfaces.nsIURIFixup);
        url = URIFixup.getFixupURIInfo(
                  linkText,
                  URIFixup.FIXUP_FLAG_NONE )?.preferredURI;
        /*url = Services.uriFixup.getFixupURIInfo(linkText).preferredURI;*/
      } catch (ex) {}
      if (url && this.isValidTld(url)) {
        gContextMenu.onPlainTextLink = true;
        gContextMenu.linkURL = url ? url.spec : null;
        gContextMenu.linkText = url ? linkText : "";
        
        let isMailtoInternal = false;
        if (gContextMenu.onMailtoLink) {
          let mailtoHandler = Cc[
        ...
        if (gContextMenu.contentData.userContextId) {
          inContainer = true;
        }
        let shouldShow =
          gContextMenu.onSaveableLink || isMailtoInternal || gContextMenu.onPlainTextLink;
        let isWindowPrivate = PrivateBrowsingUtils.isWindowPrivate(window);
        let showContainers =
          Services.prefs.getBoolPref("privacy.userContext.enabled") &&
          ContextualIdentityService.getPublicIdentities().length;
        gContextMenu.showItem("context-openlink", shouldShow && !isWindowPrivate);
        gContextMenu.showItem(
          "context-openlinkprivate",
          shouldShow && PrivateBrowsingUtils.enabled
        );
        gContextMenu.showItem("context-openlinkintab", shouldShow && !inContainer);
        gContextMenu.showItem("context-openlinkincontainertab", shouldShow && inContainer);
        gContextMenu.showItem(
          "context-openlinkinusercontext-menu",
          shouldShow && !isWindowPrivate && showContainers
        );
        gContextMenu.showItem("context-openlinkincurrent", gContextMenu.onPlainTextLink);
      }
    }
  }
}
