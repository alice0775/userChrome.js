...
// @version        2026/03/26 fix menu click handler
...
// --- config ---
    editor: (AppConstants.platform == "win") 
             ...
             : ((AppConstants.platform == "linux")
               ?"/usr/bin/gedit" /*"/usr/bin/xed"*/ /*"/usr/bin/featherpad"*/
               : ""),
// --- config ---
    _statusDisplay: null,
    ...
    onpopup: function(event){
      ...
      menuitem.toggleAttribute('checked', !flag);
      ...
      menuitem.toggleAttribute('checked', !userChrome_js.dirDisable['*']);
      menuitem.dirName = '*';
      ...
      for(var j = 0, lenj = userChrome_js.arrSubdir.length; j < lenj; j++){
        ...
        menuitem.setAttribute('type', 'checkbox');
        menuitem.toggleAttribute('checked', !userChrome_js.dirDisable[dirName]);
        menuitem.dirName = dirName;
        menupopup.appendChild(menuitem);
        menuitem.addEventListener('command', (event) => userChromejs.chgDirStat(event.target.dirName));
        ...
        for(var i = 0, len = userChrome_js.scripts.length; i < len; i++){
          ...
          if(script.dir != dirName) continue;
            ...
            menuitem.toggleAttribute('checked',!userChrome_js.scriptDisable[script.filename] );
            if(script.description)
              menuitem.setAttribute('tooltiptext',script.description);
            menuitem.script = script;
            menupopup.appendChild(menuitem);
            menuitem.addEventListener('command', (event) => userChromejs.chgScriptStat(event.target.script.filename));
    ...
    clickScriptMenu: function(event){
      if(event.button==1){
        ...
        event.target.toggleAttribute('checked',!userChrome_js.scriptDisable[event.target.script.filename] );
