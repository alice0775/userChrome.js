...
window.serachWP_modoki = {
  ...
  ICU4X_SEGMENTER: false,  // TinySegmenterに代えてicu4x.segmenterを使う デフォルト off
  // end config

  get _newSearchWidget() {
  ...
  get _textbox(){
    if(!this.searchbar)
      return null;
    return this.searchbar.textbox || this.searchbar.inputField;
  },

  get searchbar(){
    return this._newSearchWidget ? document.getElementById('searchbar-new') : document.getElementById('searchbar');
  },

  get finder(){
    return gBrowser.selectedTab.linkedBrowser.finder;
  },
  ...
  _getTokenOnMousePosition: function(event) {
  ...
  toggleHighlight: function(event) {
    ...
    if (finder.searchString)
      term = finder.searchString;
    else 
      term = this.searchbar.value;
    finder.onHighlightAllChange(this.AUTOHIGHLIGHT);
    if (term)
      finder.highlight(this.AUTOHIGHLIGHT, term, false);

    Services.prefs.setBoolPref("userChrome.searchWP_modoki.highlightAll", this.AUTOHIGHLIGHT);
  },
...
var ucjs_Tokenizer = new function() {
  ...
  this.getByOffset = function( input, offset ) {
    ...
    for (var i = 0; i < terms.length; ++i ) {
      ...
      var term = terms[i];
      while (true) {
        var index = input.indexOf(term, s);
        if (index < 0) {
          break;
        }
        if ( index <= offset && offset <= index + term.length) {
          return {"value" : term, "index": index};
        }
        s = index + term.length;
      }
    }
  ...
  if (!serachWP_modoki.ICU4X_SEGMENTER /*TinySegmenter*/) {
      var segmenter = new TinySegmenter();                 // インスタンス生成
      var result = segmenter.segment(aSearchString.replace(/[\s　]+/g,' ' + DELIMITER + ' '));  // 単語の配列が返る
  } else {
    const segmenterJa = new Intl.Segmenter("ja-JP", { granularity: "word" });
    const segments = segmenterJa.segment(aSearchString.replace(/[\s　]+/g,' ' + DELIMITER + ' '));
    var result0 = Array.from(segments);
    var result = [];
    for (var i = 0; i < result0.length; i++){
    result.push(result0[i].segment);
    }
  }
//userChrome_js.debug(result);
      ...
      for (var i = 0; i < aaa.length; i++){
        aaa[i] = aaa[i].replace(/^\s+|\s+$/,'');
        j = aaa[i].search(/["']/);
        if (j > -1){
          if (j == 0){
            result.push(aaa[i].substr(0,1));
            result.push(aaa[i].substr(1));
          } else {
            result.push(aaa[i].substr(0,j));
            result.push(aaa[i].substr(j));
          }
        } else {
          result.push(aaa[i]);
        }
      }
      //together within quotations
      aaa =[];
      var quote = false;
      var quoteType = null;
      var txt ="";
      ...
      for (var i = 0; i < result.length; i++){
        if (result[i] == "\"" || result[i] == "'"){
          if (!quote){
            ...
            prev = "quoteType";
            continue;
          } else if(quote && result[i] == quoteType){
            //end quatation
            quote = false;
            quoteType = null;
            aaa.push(txt);
        ...
        if (!quote){
          var x = prev + result[i];
          if (!!prev && !!result[i] && result[i] != DELIMITER &&
              (/^[+-.,/:.\d]+$/.test(result[i]) ||
               /^[＋－，．／：．０-９]+$/.test(result[i]) ) &&
              (/^[+-.,\d]+[+-,/\d]*$/.test(x) ||
               /^[+-.,\d]*[+-,:\d]+$/.test(x) ||
               ...
               /^[＋－，．０-９]*[＋－，：０-９]+$/.test(x) ||
               /^[＋－，．０-９]*[＋－，．０-９]+$/.test(x)) ){
            prev = aaa[aaa.length - 1] + result[i];
            aaa[aaa.length - 1] = prev;
          } else if (!!prev && !!result[i] && result[i] != DELIMITER &&
                    (/^\w*([._]*\w*)+$/.test(result[i]) ||
                     /^[ａ-ｚＡ-Ｚ０-９]*([．＿]*[ａ-ｚＡ-Ｚ０-９]*)+$/.test(result[i]) ) &&
                    (/^\w*([._]*\w*)+$/.test(x) ||
                     /^[ａ-ｚＡ-Ｚ０-９]*([．＿]*[ａ-ｚＡ-Ｚ０-９]*)+$/.test(x)) ){
             prev = aaa.pop() + result[i];
            aaa.push(prev);
          } else {
            aaa.push(result[i]);
            prev = result[i];
      ...
      //remove space and unique
      result =[];
      for (var i = 0; i< aaa.length; i++){
        if (!/^(\s)$/.test(aaa[i]) && !!aaa[i] && aaa[i] != DELIMITER){
          if (result.indexOf(aaa[i]) == -1)
            result.push(aaa[i].replace(new RegExp(DELIMITER,'g'),' ').replace(/^\s+|\s+$/,''));
        }
      }
      return result;
    }
