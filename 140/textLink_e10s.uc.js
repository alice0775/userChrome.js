...
// @version        2025/08/18 00:00 check visibility and document flow(Bug 1981023)
...
function ucjs_textlink(event) {
  ...
  function getContainingBlock(el) {
    ...
    while (parent && win.getComputedStyle(parent).position === 'static') {
      parent = parent.parentElement;
    }
    // position:fixed の場合、包含ブロックは存在しないが、ここでは便宜上 null を返す
    return parent;
  }
  function isWithinSamePositionedElement(element) {
    const parent = element.parentNode;
    if (!parent) {
      return false; // 親要素がない場合はfalse
    }
    const win = parent.ownerDocument.defaultView;
    const parentPosition = win.getComputedStyle(parent).position;

    return parentPosition !== 'static';
  }
...
debug("root " + root.textContent);
  ...
  //親ブロック要素の文字列をすべて得る
    ...
    allowedParents.push(root.localName);
...
debug("candidates.snapshotLength " + candidates.snapshotLength);
    ...
    var containingBlock = getContainingBlock(sOyaNode);
    if (candidates.snapshotLength < 1) {
      ...
      if (i >= 0) {
        for (var i = i1; i >= 0 ; i--){
          if (!candidates.snapshotItem(i).parentNode.checkVisibility({opacityProperty: true, visibilityProperty: true})) {
            continue;
          }
          let containingBlock2 = getContainingBlock(candidates.snapshotItem(i));
          if (containingBlock != containingBlock2) {
            continue;
          }
      ...
      //レンジより後ろにある文字列
      for(var i = i1 + 1, len = candidates.snapshotLength; i < len; i++) {
debug("snapshotItem("+i+") "+ candidates.snapshotItem(i).nodeValue);
debug("visible? " + candidates.snapshotItem(i).parentNode.checkVisibility({opacityProperty: true, visibilityProperty: true}));
        if (!candidates.snapshotItem(i).parentNode.checkVisibility({opacityProperty: true, visibilityProperty: true})) {
          continue;
        }
        let containingBlock2 = getContainingBlock(candidates.snapshotItem(i));
debug("same block? " + !(containingBlock != containingBlock2));
        if (containingBlock != containingBlock2) {
          continue;
        }
...
function ucjs_textlink_onclick(event) {
  clickCount++;

  if (clickCount === 1) {
    timer = event.target.ownerDocument.defaultView.setTimeout(function() {
      if (clickCount === 1) {
        // シングルクリック時の処理 (何もしない)
    ...
    event.target.ownerDocument.defaultView.clearTimeout(timer);
    clickCount = 0;
    // トリプルクリック以上の処理 (何もしない)
  }
}
...
function ucjs_textlink_main() {
  window.messageManager.loadFrameScript(
     'data:application/javascript,'
      ...
      + encodeURIComponent(ucjs_textlink_onclick.toSource())
      + encodeURIComponent('let timer = null;let clickCount = 0;addEventListener("click", ucjs_textlink_onclick, false);')
