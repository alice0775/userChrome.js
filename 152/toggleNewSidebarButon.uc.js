...
(function() {
    try {
        CustomizableUI.createWidget({
            ...
            onCommand(event) {
              let commandID = SessionStore.getCustomWindowValue(event.target.documentGlobal, "userChrome.sidebar2");
                //Services.prefs.getStringPref("userChrome.sidebar2", event.target.documentGlobal.SidebarController.DEFAULT_SIDEBAR_ID);
              commandID = event.target.documentGlobal.SidebarController.lastOpenedId || commandID;
              if (event.target.documentGlobal.SidebarController.isOpen && typeof AutohideSidebar != "undefined" && event.target.documentGlobal.AutohideSidebar.enabled) {
                if (event.target.documentGlobal.AutohideSidebar.hidden) {
                  event.target.documentGlobal.AutohideSidebar.show();
                } else {
                  event.target.documentGlobal.AutohideSidebar.hide();
                }
              }  else if (event.target.documentGlobal.SidebarController.isOpen && typeof ucjs_expand_sidebar != "undefined") {
                event.target.documentGlobal.ucjs_expand_sidebar.toggleSidebar(commandID);
              }  else {
                  event.target.documentGlobal.SidebarController.toggle(commandID);
                  event.target.checked = event.target.documentGlobal.SidebarController.isOpen;
              }
            },
