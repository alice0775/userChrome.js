  ...
  init: function() {
    try {
      CustomizableUI.createWidget({ //must run createWidget before windowListener.register because the register function needs the button added first
        ...
        onCommand(event) {
          event.target.documentGlobal.ddwin_button.launchWithSelectedText()
        },
...
*/
        this.exec(ddwinPath, ",2," + ddwinGroup + ",G1," + query);
    }
  },
...
*/
  ...
  exec: function(aFilePath, aArgs, blocking){
    ...
    try{
      ...
      process.runw(blocking, argArray, argArray.length);
    }catch(e){
      alert(e);
      return false;
    }
