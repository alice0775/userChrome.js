...
window.openLibraryContextMenu = {
  ...
  init: function () {
    ...
    menuitem.addEventListener("command", (event) => {event.target.documentGlobal.openLibraryContextMenu.showOrganizer(menuitem.parentNode.triggerNode)});
    //menuitem.setAttribute("oncommand", "  Services.console.logStringMessage(this.parentNode.triggerNode);openLibraryContextMenu.showOrganizer(this.parentNode.triggerNode);");
  ...
  getParentFolderByItemId: function(aNode){
    ...
    var itemType = PlacesUtils.nodeIsFolderOrShortcut(aNode) ||
                   PlacesUtils.nodeIsTagQuery(aNode) ? "folder" : "bookmark";
    var concreteId = PlacesUtils.getConcreteItemGuid(aNode);
    ...
    //if (PlacesUtils.nodeIsBookmark(aNode)) {
      parentFolderId = openLibraryContextMenu.getFolderIdForItem(itemId);
    //}
  ...
  selectNode: function(aNode) {
    ...
    var isFolder = PlacesUtils.nodeIsFolderOrShortcut(aNode);
    if (isFolder) {
