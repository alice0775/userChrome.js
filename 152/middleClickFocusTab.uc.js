...
// @version             2026/03/29 searchbar-new
...
function middleClickUIFocusTab() {
  ...
  function onmousedown(event) {
    ...
    Services.console.logStringMessage(event.target.nodeName);
    if(!event.isTrusted) return;
    ...
    if (_state == MCLICK_STATE_NONE) {
      _pref1 = Services.prefs.getBoolPref("browser.search.openintab", false);
      Services.prefs.setBoolPref("browser.search.openintab", false);
      _pref2 = Services.prefs.getBoolPref("browser.urlbar.openintab", false);
      Services.prefs.setBoolPref("browser.urlbar.openintab", false);
    }
  ...
  function onmouseup(event) {
    ...
    window.setTimeout(() => {
        ...
        _target = null;
        Services.prefs.setBoolPref("browser.search.openintab", _pref1);
        Services.prefs.setBoolPref("browser.urlbar.openintab", _pref2);
    }, 10);
  }
  ...
  function onclick(event) {
    ...
    if (_state == MCLICK_STATE_HOLD && 
      ...
      let loc = _target.documentGlobal.location;
      ...
      // handle clicks on go button.
        ...
        Services.console.logStringMessage('dispatchEvent ' + e.type);
        Services.console.logStringMessage('_target ' + _target.localName);
        
      } else if(_target.closest(".searchmode-switcher-installed")) {
  ...
  function findLink(aEvent) {
    ...
    node = 
       ...
       node.closest('.searchmode-switcher-installed') ||
