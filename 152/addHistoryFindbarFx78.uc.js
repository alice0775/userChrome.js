...
// @version        2025/09/21 漢字等の場合1文字でも覚える
...
const addHistoryFindbar78 = {
  ...
  init: function() {
    ...
    .findBar-history-dropmarker, .clearFindbar-button {
      ...
      &:hover {
        background-color: var(--toolbarbutton-background-color-hover, rgba(190,190,190,.2)) !important;
        outline: none;
      }
      &:focus-visible {
...
*/
  ...
  addDropMarker: function(findbar) {
    ...
    menupopup.addEventListener("popupshowing", (event) => addHistoryFindbar78.onpopupshowing(event));
    menupopup.addEventListener("command", (event) => addHistoryFindbar78.copyToFindfield(event));
    menupopup.addEventListener("click", (event) => addHistoryFindbar78.onclick(event));
    menupopup.addEventListener("mouseup", (event) => addHistoryFindbar78.shouldPreventHide(event));
/*
...
*/
    ...
    if (!this.DeleteButton)
      return;
		...
		ClearFindbarBtn.addEventListener("command", (event) => clearFindbarTweak.onCommand());
  },
    ...
    switch (event.type) {
      ...
      case 'findagain':
        ...
        if (gFindBar._findField.lastInputValue != text &&
            encodeURI(text).split(/%..|./).length - 1 > 1) {
  ...
  onpopupshowing: function(event, menupopup) {
    ...
    element.addEventListener("command", (event) => {event.stopPropagation(); addHistoryFindbar78.clearHistory();});
