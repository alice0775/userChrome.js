  ...
  init: function() {
    ...
    try {
      CustomizableUI.createWidget({
        ...
        onCommand(event) {
          event.target.documentGlobal.memoryMinimizationButton.doMinimize(event);
        },
        onClick(event) {
          if (event.button == 1)
            event.target.documentGlobal.memoryMinimizationButton.doMinimize(event);
...
if (location.href == "chrome://browser/content/browser.xhtml") {
  ...
  Services.console.logStringMessage("killing start(force)");
  setTimeout(() => {
    ...
    for(let closeButton of closeButtons) {
      ...
      for (let childRow = row.nextSibling;
           childRow && childRow.classList.contains("window");
           childRow = childRow.nextSibling ) {
        let win = childRow.win;
        if (!win || !win.tab || !win.tab.tab) {
          canKill = false;
          break;
        }
